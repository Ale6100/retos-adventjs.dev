...
function decodeFilename(filename: string): string {
  const indiceIniciaNombre = filename.indexOf('_');
  const indiceTerminaNombre = filename.lastIndexOf('.');
  return filename.slice(indiceIniciaNombre+1, indiceTerminaNombre)
}
